...
int query_processor::process_qry_entrust(base::dictionary& dict)
{
	...
	/* check orderid, and orderid should be specified */
	std::string orderid = dict["orderid"].string_value();
	if (orderid.empty()) {
		TRACE_ERROR(AT_TRACE_TAG, NAUT_AT_E_ORDERID_SHOULD_SPECIFIED,
				"orderid of the entrust query should be specified");
		ASSIGN_AND_CHECK_LABEL(ret, NAUT_AT_E_ORDERID_SHOULD_SPECIFIED, end);
	}

	/* check whether the orderid existed */
	char sql[2048];
	sprintf(sql, "select * from entrust where orderid='%s' and entrust_type=%d",
	...
	if (db_conn->_conn->get_count() == 0)
	{
		TRACE_ERROR(AT_TRACE_TAG, NAUT_AT_E_ORDER_NOT_EXIST,
				"order '%s' does not exist in the table, order: %s",
				dict["orderid"].string_value().c_str(), dict.to_string().c_str());
		ASSIGN_AND_CHECK_LABEL(ret, NAUT_AT_E_ORDER_NOT_EXIST, end);
	}

	/* send successful entrust-query response */
	base::dictionary* pdict = new base::dictionary(dict);
	ref_dictionary* rd = new ref_dictionary(pdict);

	(*pdict)["cmd"] = ATPM_CMD_RSP_ENTRUST;
	(*pdict)["count"] = (int)0;
	(*pdict)["result"] = new base::darray();
	(*pdict)["error_code"] = (int)0;

	while (db_conn->_conn->fetch_row()) {
		base::dictionary record;
		record["systemno"] = db_conn->_conn->get_string("systemno");
		record["entrustno"] = db_conn->_conn->get_string("entrustno");
		record["entrust_type"] = db_conn->_conn->get_long("entrust_type");
		record["entrust_amount"] = db_conn->_conn->get_long("entrust_amount");
		record["entrust_price"] = db_conn->_conn->get_string("entrust_price");
		record["order_type"] = db_conn->_conn->get_long("order_type");
		record["direction"] = db_conn->_conn->get_long("direction");
		record["code"] = db_conn->_conn->get_string("code");
		record["excode"] = db_conn->_conn->get_string("excode");
		record["order_status"] = db_conn->_conn->get_long("order_status");
		record["error_code"] = db_conn->_conn->get_long("error_code");
		record["error_msg"] = db_conn->_conn->get_long("error_msg");

		(*pdict)["count"] = (*pdict)["count"].int_value() + 1;
		(*pdict)["result"].array_value()->append(record);
	}
...
int query_processor::process_qry_deal(base::dictionary& dict)
{
	...
	/* check systemno, which must be specified on the request to trade server */
	std::string systemno = "";
	char sql[2048];
	sprintf(sql, "select * from entrust where orderid='%s' and entrust_type=%d",
			dict["orderid"].string_value().c_str(), dict["entrust_type"].int_value());
	...
	if (db_conn->_conn->get_count() == 0) {
		TRACE_WARNING(AT_TRACE_TAG, "order '%s' does not exist in the table, order: %s",
				dict["orderid"].string_value().c_str(), dict.to_string().c_str());
		ASSIGN_AND_CHECK_LABEL(ret, NAUT_AT_E_ORDER_NOT_EXIST, end);
	}
	else {
		db_conn->_conn->fetch_row();
		systemno = db_conn->_conn->get_string("systemno");
	}
	...
	(*pdict)["order_type"] = db_conn->_conn->get_long("order_type");
